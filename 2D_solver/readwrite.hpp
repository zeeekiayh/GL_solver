...
void WriteToFile(const T_vector& solution_vector, std::string file_name, int Nop, const in_conditions cond) {
	std::ofstream data (file_name); // open the file for writing
	if (data.is_open()) {           // if opening was successful...

		int gsize = solution_vector.size()/Nop;
		...
		for (int i = 0; i < gsize; i++) { // loop through each grid point
			...
			for (int n = 0; n < Nop; n++) { // put the element of each OP component on the same line
