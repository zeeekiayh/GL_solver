...
inline T_vector get_rhs(T_vector h2_times_rhs_bulk, T_vector rhsBC){
...
class Solver {
	private:
		int grid_size, vect_size, iter;
		double h2, err, prev_error, percent_delta_err, rp;
		SpMat_cd M;
		in_conditions cond;
		...
		bool ready;
	public:
		Solver(SpMat_cd M, in_conditions cond, SC_class*&SC) {
			this->M=M;
			this->cond=cond;
			this->SC=SC;

			grid_size=cond.SIZEu * cond.SIZEv;
			vect_size=cond.Nop * grid_size;
			h2 = cond.STEP*cond.STEP;
			...
			if (solver.info() == Eigen::Success) {
				cout << "\tSolver: successfully built" << endl;
				ready=true;
			}
		...
		void Solve(T_vector & f, vector<int> no_update, T_vector rhsBC) {
			if (!ready) {
				cout << "ERROR: solver is not ready!" << endl;
				return;
			}
			...
			// adaptive relaxation
			prev_error = 0.;
			rp = cond.rel_p;//, rpMax = 0.1;//, rpMin = 0.001;
