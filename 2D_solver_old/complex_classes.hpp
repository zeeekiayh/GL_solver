...
//  for complex, 3-component GL solver
   ...
   class Three_Component_GL_Solver<dcomplex> : public GL_Solver<dcomplex> {
      ...
      {
         // TODO: fix the indexing of the g vector!! Need to use the ID function!
         for (int n = 0; n < OP_size; n++) {
            if (etaBC[n].typeB == string("D")) {
               for (int i = cond.SIZEu*(cond.SIZEv-1) + n*mSize; i < (n+1)*mSize; i++)
                  g(i) = etaBC[n].valB;
            }
            if (etaBC[n].typeT == string("D")) {
               for (int i = n*mSize; i < cond.SIZEu + n*mSize; i++)
                  g(i) = etaBC[n].valT;
            }
            // TODO: add left and right here!
         }
         return g;
      }
